...
def mel_spec2power_spec(mel_spec, sr=hparams['sample_rate'],
                        n_fft=hparams['n_fft'],
                        num_mels=hparams['num_mels'],
                        fmin=hparams['fmin'],
                        fmax=hparams['fmax']):
    # mel_spec should be of shape [time, n_mels]
    global _mel_basis
    _mel_basis = librosa.filters.mel(
        sr, n_fft, n_mels=num_mels,
        fmin=fmin, fmax=fmax) if _mel_basis is None else _mel_basis  # [n_mels, 1+n_fft/2]
    power_spec = np.dot(np.linalg.pinv(_mel_basis), mel_spec.T).T
    power_spec = np.maximum(1e-10, power_spec)  # force power_spec to be positive
    return power_spec


def log_mel_spec2wav(log_mel_spec):
    mel_spec = db2power(log_power_denormalize(log_mel_spec))
    power_spec = mel_spec2power_spec(mel_spec)
    wav_arr = griffin_lim(power_spec**0.5)
    return wav_arr
...
# def display_test(wav_f, n_fft=hparams['n_fft'],
#                  hop_len=hparams['hop_length'],
#                  win_len=hparams['win_length'],
#                  window=hparams['window']):
...
#     pow_spec = spec_dict['power']
#     log_power = power2db(pow_spec)
#     normalized = log_power_normalize(log_power)
#     print(np.mean(pow_spec, axis=-1))
#     plt.figure()
#     plt.subplot(3, 1, 1)
#     plt.imshow(pow_spec.T, origin='lower')
#     plt.colorbar()
#     plt.title('Power spectrogram')
#     plt.subplot(3, 1, 2)
#     plt.imshow(log_power.T, origin='lower')
#     plt.colorbar(format='%+2.0f dB')
#     plt.title('Log-Power spectrogram')
#     plt.subplot(3, 1, 3)
#     plt.imshow(normalized.T, origin='lower')
#     plt.colorbar()
...
#
#
# def display_test_from_stft(mag_stft_f):
#     import matplotlib.pyplot as plt
#     mag_spec = np.load(mag_stft_f)
#     pow_spec = mag_spec**2
#     log_power = power2db(pow_spec)
#     normalized = log_power_normalize(log_power)
#     print(np.mean(pow_spec, axis=-1))
...
#
#
# def compare_test(wav_f, stft_f, n_fft=hparams['n_fft'],
#                  hop_len=hparams['hop_length'],
#                  win_len=hparams['win_length'],
#                  window=hparams['window']):
#     import matplotlib.pyplot as plt
...
#     spec_dict = spectrogram(wav_arr, n_fft=n_fft, hop_len=hop_len,
#                             win_len=win_len, window=window)
#     pow_spec = spec_dict['magnitude']
#     print(pow_spec.min(), pow_spec.max(), pow_spec.mean())
#     predicted_stft = np.load(stft_f)
#     print(predicted_stft.min(), predicted_stft.max(), predicted_stft.mean())
#     print(pow_spec.shape, predicted_stft.shape)
#     plt.figure()
#     plt.subplot(211)
#     plt.imshow(pow_spec.T, origin='lower')
#     plt.colorbar()
...
#     plt.subplot(212)
#     plt.imshow(predicted_stft.T, origin='lower')
#     plt.colorbar()
...
def logmelspec2wav_test(wav_path):
    ...
    wav_arr = log_mel_spec2wav(np.squeeze(logmel))
    wav_arr = deemphasis(wav_arr)
    write_wav('resyn.wav', wav_arr, sr=16000)
    return


# if __name__ == '__main__':
#     logmelspec2wav_test('00002031.wav')
# display_test('00000001.wav')
# griffin_lim_test('00000001.wav')
# stft2wav_tf_test('../share/zhiling/stft-513/00000502.npy',
#                  '../share/zhiling/statistics/stft-513/mean.npy',
#                  '../share/zhiling/statistics/stft-513/std.npy')
